...
def render() -> None:
    ...
    company_counts = company_counts[company_counts["company_name"].isin(top_companies)].copy()

    heatmap_df = company_counts.pivot(index="company_name", columns="name", values="count").fillna(0)
    ...
    st.caption("※ 상위 N개 기업만 히트맵에 표시하여 기업별 기술 프로파일을 쉽게 확인할 수 있습니다.")
